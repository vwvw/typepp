...
class JetStream(browserbench.BrowserBench):
  ...
  def RunAndExtractMeasurements(self, driver, optargs):
    ...
    WebDriverWait(driver,
                  timeout=600).until(lambda driver: not _IsLoading(driver))
    ...
    WebDriverWait(driver, timeout=36000).until(lambda driver: _IsDone(driver))
    results =  _GetResults(driver, optargs)
    PROCNAME = "chrome"
