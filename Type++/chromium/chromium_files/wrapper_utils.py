...
def RunLinkWithOptionalMapFile(command, env=None, map_file=None):
  ...
  # NICOLAS FIX
  command = [x if "-Wl,-z,defs" != x else "" for x in command ]
  command += ' -Wl,-rpath,REPLACE_LIBCXXABI/lib \
-LREPLACE_LIBCXXABI/lib \
-Wl,-rpath,REPLACE_LIBUNWIND/lib \
-LREPLACE_LIBUNWIND/lib \
-Wl,-rpath,REPLACE_LIBCXX1/lib \
-LREPLACE_LIBCXX1/lib \
-stdlib=libc++ -lc++abi -lc++ -lunwind -ljsoncpp -Wno-deprecated-copy -Wl,--allow-shlib-undefined'.split(" ")
